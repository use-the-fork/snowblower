{
  ...
  flake.flakeModules.integrations = {
    options.perSystem = flake-parts-lib.mkPerSystemOption ({
      ...
      cfg = config.snowblower.integrations.git-hooks;
    in {
      options.snowblower.integrations.git-hooks = lib.mkOption {
      ...
      config.snowblower = lib.mkIf ((lib.filterAttrs (_id: value: value.enable) cfg.hooks) != {}) {
