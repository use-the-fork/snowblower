...
in
  pkgs.writeShellScript "init-yarn.sh" ''
    function _snow_blower-yarn-install()
    {
      ...
      local ACTUAL_YARN_CHECKSUM="${cfg.yarn.package.version}:$(${pkgs.nix}/bin/nix-hash --type sha256 ${lib.optionalString (cfg.directory != config.snow-blower.paths.root) ''"${cfg.directory}/"''}yarn.lock)"
      ...
      then
        if ${cfg.yarn.package}/bin/yarn install ${lib.optionalString (cfg.directory != config.snow-blower.paths.root) "--cwd ${cfg.directory}"}
        then
    ...
    then
      echo "No package.json found${lib.optionalString (cfg.directory != config.snow-blower.paths.root) ''"in ${cfg.directory}"''}. Run '${lib.optionalString (cfg.directory != config.snow-blower.paths.root) ''"cd ${cfg.directory}/ && "''}yarn init' to create one." >&2
    else
      _snow_blower-yarn-install
    fi
  ''
