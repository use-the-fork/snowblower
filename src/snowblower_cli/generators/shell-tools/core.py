...
class AllTools(BaseGenerator):
    ...
    def handle(self, pending_generator: GeneratorOutput) -> GeneratorOutput:
        ...
        """
        for tool_key, tool_config in self.config.languages.items():
            try:
                # Import the language module dynamically
                module_path = f"snowblower_cli.generators.tools.{tool_key}.core"
                class_name = f"{tool_key.capitalize()}Language"
                ...
                rich.print(f"Failed to import language module for {tool_key}: {e}")
