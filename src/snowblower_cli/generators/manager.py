...
from snowblower_cli.utils.file_handlers import (
    FileHandlerInput, FileType, JsonHandler, TomlHandler, YamlHandler
)
                ...
                if isinstance(file_input, FileHandlerInput):
                    try:
                        ...
                        if file_input.file_type == FileType.JSON:
                            ...
                            logger.error(f"Unsupported file type: {file_input.file_type}")
                            continue
                        ...
                        logger.info(f"Wrote {file_input.file_type.name} configuration to {file_path}")
