...
in {
  ...
  isDag = dag: builtins.isAttrs dag && all snowblower.dag.isEntry (builtins.attrValues dag);
  ...
  entryAnywhere = snowblower.dag.entryBetween [] [];

  entryAfter = snowblower.dag.entryBetween [];
  entryBefore = before: snowblower.dag.entryBetween before [];
  ...
  entriesBetween = tag: let
    ...
    in
      if entries == []
      then snowblower.dag.empty
      else if length entries == 1
      then {
        "${name}" = snowblower.dag.entryBetween before after (head entries);
      }
      else
        {
          "${name}" = snowblower.dag.entryAfter after (head entries);
        }
        // go (i + 1) before [name] (tail entries);
  in
    go 0;

  entriesAnywhere = tag: snowblower.dag.entriesBetween tag [] [];
  entriesAfter = tag: snowblower.dag.entriesBetween tag [];
  entriesBefore = tag: before: snowblower.dag.entriesBetween tag before [];
}
