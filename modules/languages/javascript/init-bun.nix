...
in
  pkgs.writeShellScript "init-bun.sh" ''
    function _snow_blower-bun-install()
    {
      ...
      local ACTUAL_BUN_CHECKSUM="${cfg.bun.package.version}:$(${pkgs.nix}/bin/nix-hash --type sha256 ${lib.optionalString (cfg.directory != config.snow-blower.paths.root) ''"${cfg.directory}/"''}yarn.lock)"
      ...
      then
        if ${cfg.bun.package}/bin/bun install --yarn ${lib.optionalString (cfg.directory != config.snow-blower.paths.root) "--cwd ${cfg.directory}"}
        then
    ...
    then
      echo "No package.json found${lib.optionalString (cfg.directory != config.snow-blower.paths.root) ''"in ${cfg.directory}"''}. Run '${lib.optionalString (cfg.directory != config.snow-blower.paths.root) ''"cd ${cfg.directory}/ && "''}bun init' to create one." >&2
    else
