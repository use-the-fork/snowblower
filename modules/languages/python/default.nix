  ...
  flake.flakeModules.languages = {
    options.perSystem = flake-parts-lib.mkPerSystemOption ({
      ...
      initVenvScript = pkgs.writeShellScript "init-venv.sh" ''
        ...
        # existing virtual environment. For instance if snowblower was started within an venv.
        unset VIRTUAL_ENV

        VENV_PATH="${config.snow-blower.env.PROJECT_STATE}/${lib.optionalString (cfg.directory != config.snow-blower.paths.root) ''"${cfg.directory}/"''}venv"

        profile_python="$(${readlink} ${package.interpreter})"
        snowblower_interpreter_path="$(${pkgs.coreutils}/bin/cat "$VENV_PATH/.snowblower_interpreter" 2> /dev/null || echo false )"
        venv_python="$(${readlink} "$snowblower_interpreter_path")"
        ...
        then
          ...
          [ -f "${config.snow-blower.env.PROJECT_STATE}/poetry.lock.checksum" ] && rm ${config.snow-blower.env.PROJECT_STATE}/poetry.lock.checksum
        ''}
          ${
          if cfg.uv.enable
          then ''
          ...
          ''
            ...
            echo ${package.interpreter} -m venv ${
              if builtins.isNull cfg.version || lib.versionAtLeast cfg.version "3.9"
              then "--upgrade-deps"
              else ""
            } "$VENV_PATH"
          ...
          echo "${package.interpreter}" > "$VENV_PATH/.snowblower_interpreter"
        fi
        ...
        if [ -n "$requirements" ]
          then
            snowblower_requirements_path="$(${pkgs.coreutils}/bin/cat "$VENV_PATH/.snowblower_requirements" 2> /dev/null|| echo false )"
            snowblower_requirements="$(${readlink} "$snowblower_requirements_path")"
            if [ -z $snowblower_requirements ] || [ $snowblower_requirements != $requirements ]
              then
                echo "${requirements}" > "$VENV_PATH/.snowblower_requirements"
                ${
          if cfg.uv.enable
          then ''
      ...
      initPoetryScript = pkgs.writeShellScript "init-poetry.sh" ''
        function _snowblower_init_poetry_venv
        {
          # Make sure any tools are not attempting to use the Python interpreter from any
          # existing virtual environment. For instance if snowblower was started within an venv.
          unset VIRTUAL_ENV
        ...
        function _snowblower_poetry_install
        {
          local POETRY_INSTALL_COMMAND=(${cfg.poetry.package}/bin/poetry install --no-interaction ${lib.concatStringsSep " " cfg.poetry.install.arguments} ${lib.optionalString (cfg.directory != config.snow-blower.paths.root) ''--directory=${cfg.directory}''})
          ...
          local ACTUAL_POETRY_CHECKSUM="${package.interpreter}:$(${pkgs.nix}/bin/nix-hash --type sha256 "$PROJECT_ROOT"/${lib.optionalString (cfg.directory != config.snow-blower.paths.root) ''${cfg.directory}/''}pyproject.toml):$(${pkgs.nix}/bin/nix-hash --type sha256 "$PROJECT_ROOT"/${lib.optionalString (cfg.directory != config.snow-blower.paths.root) ''${cfg.directory}/''}poetry.lock):''${POETRY_INSTALL_COMMAND[@]}"
          local POETRY_CHECKSUM_FILE="$PROJECT_ROOT"/${lib.optionalString (cfg.directory != config.snow-blower.paths.root) ''${cfg.directory}/''}.venv/poetry.lock.checksum
        ...
        if [ ! -f "$PROJECT_ROOT"/${lib.optionalString (cfg.directory != config.snow-blower.paths.root) ''${cfg.directory}/''}pyproject.toml ]
        then
          echo "No pyproject.toml found. Run 'poetry init' to create one." >&2
        else
          _snowblower_init_poetry_venv
          ${lib.optionalString cfg.poetry.install.enable ''
          _snowblower_poetry_install
        ''}
          ${lib.optionalString cfg.poetry.activate.enable ''
          source "$PROJECT_ROOT"/${lib.optionalString (cfg.directory != config.snow-blower.paths.root) ''${cfg.directory}/''}.venv/bin/activate
        ''}
        fi
      '';
    in {
      options.snow-blower.languages.python = {
        ...
        libraries = lib.mkOption {
          ...
          default = ["${config.snow-blower.paths.dotfile}/profile"];
        ...
        venv.quiet = lib.mkOption {
          type = lib.types.bool;
          default = false;
          description = "Whether `pip install` should avoid outputting messages during snowblower initialisation.";
        };
        ...
        poetry = {
          ...
          install = {
            enable = lib.mkEnableOption "poetry install during snowblower initialisation";
            arguments = lib.mkOption {
              type = lib.types.listOf lib.types.str;
              default = [];
              description = "Command line arguments pass to `poetry install` during snowblower initialisation.";
              internal = true;
            };
            ...
            quiet = lib.mkOption {
              type = lib.types.bool;
              default = false;
              description = "Whether `poetry install` should avoid outputting messages during snowblower initialisation.";
            };
            groups = lib.mkOption {
              type = lib.types.listOf lib.types.str;
              default = [];
      ...
      config.snow-blower = lib.mkIf cfg.enable {
        ...
        languages.python.package = lib.mkMerge [
          (lib.mkIf (cfg.version != null)
            (nixpkgs-python.packages.${pkgs.stdenv.system}.${cfg.version} or (throw "Unsupported Python version.")))
        ];
        ...
        env = lib.optionalAttrs cfg.poetry.enable {
          # Make poetry use PROJECT_ROOT/.venv
